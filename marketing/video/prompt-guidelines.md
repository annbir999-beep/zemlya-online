...
- **Дети «просачиваются» сквозь забор (из 5-28 v2):** если написать «children running on the
  lawn inside the fenced yard», модель кладёт детей ПОВЕРХ реек забора (ноги/тело сливаются с
  забором, дети видны сквозь него). Ставить детей ЯВНО в центр газона, вдали от забора,
  целиком ПЕРЕД ним: `two children playing in the OPEN CENTER of the lawn well away from the
  fence, their whole bodies and legs fully in front of and separate from the fence, NOT
  touching / NOT overlapping / NOT clipping through / NOT merging with the fence, the low dark
  slat fence stays a solid continuous UNBROKEN barrier behind them`. То же в video-промпт.
