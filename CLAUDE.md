# Sotka / Торги Земли — правила и база проекта
...
- **Источник лотов**: только torgi.gov (публичное REST API). AVITO и CIAN (скрейпинг) — НЕ лоты, а рыночные предложения для сравнения цены на странице лота; на карте не отображаются (фронт и бэк дефолтят `source=torgi_gov`).
...
### Разделение труда с агентами (НЕ дублировать постинг!)
- **Публикация роликов — НЕ моя зона.** Постингом занимается отдельный агент Анны:
  готовые ролики (после субтитров) → Notion → он ставит на постинг. **НЕ постить
  напрямую в Postmypost/соцсети** — иначе дубли (было 11.07: я поставил день 1 в
  Postmypost, агент тоже → дубли).
- **Моя зона по видео**: генерация роликов (GoldWork, см. [[goldwork-batch-generate]])
  → готовые складывать в отдельную папку для агента (путь уточнять у Анны). На этом
  стоп — дальше субтитры/Notion/постинг ведёт агент.
- Параллельные сессии видны только «по факту» — через общую auto-memory и git,
  не вживую. Перед деплоем backend-фиксов сверяться с памятью: субаренда/переуступка
  и честные метрики morning_check уже на проде (main) другой сессией — свою
  дивергентную версию с `feature/video-factory` НЕ деплоить.
...
- **AVITO/CIAN — НЕ ЛОТЫ** — это рыночные предложения для сравнения цен, на карте их быть не должно и в счётчик лотов они не входят. Правило Анны, подтверждено 27.07.2026. Реальных аукционных лотов ~12.9к (torgi.gov), а не 63к: остальное — 39.6к Avito + 10.6к ЦИАН. Любую публичную цифру «сколько у нас лотов» считать ТОЛЬКО по `source=torgi_gov`. Старая грабля «покрытие карты 6.5% из-за AVITO без КН» неактуальна: карта считается только по torgi.gov.
