...
# Расширено 2026-05-12 и 2026-07-10: были слишком узкие патерны (ловили 13 из 4024
# договоров). Добавлены поднаём/перенаём, залог права аренды, внесение в уставный
# капитал, обороты ст. 22 ЗК РФ. Текст нормализуется в parse_contract (пробелы,
# ё→е), поэтому \s+ переживает переносы строк PDF. Порядок разбора:
# forbidden > with_consent > with_notice > allowed > эвристика ст.22 (в scrape_tasks).
# Явный запрет всегда перекрывает всё.
CESSION_FORBIDDEN = [
    ...
    # добавлено 2026-07-10
    r"(?:перенайм|перенаем)\w*.{0,40}(?:не\s+допуск|запрещ|не\s+разреш)",
    r"арендатор\s+не\s+(?:вправе|имеет\s+права)\b.{0,80}(?:уступа\w+|передава\w+|переда\w+|переуступа\w+)\b.{0,40}(?:прав|обязанност|аренд)",
    r"(?:уступк\w+|переуступк\w+|передач\w+)\b.{0,60}прав\w*.{0,40}(?:не\s+допуск|запрещ|исключ)",
    r"запрещ\w+.{0,40}(?:залог\w*|в\s+залог)\s+прав\w*\s+аренд",
    r"не\s+допуска\w+\s+внесени\w+\s+прав\w*\s+аренд\w*\s+в\s+(?:устав|качестве)",
    r"права\s+и\s+обязанности\s+по\s+.{0,30}договор\w+.{0,40}не\s+(?:могут\s+быть\s+переда|подлежат\s+переда)",
]
CESSION_ALLOWED_WITH_NOTICE = [
    ...
    # добавлено 2026-07-10
    r"(?:перенайм|перенаем)\w+",
    r"передава\w+\s+(?:свои\s+)?права\s+и\s+обязанности\b.{0,60}третьим\s+лицам\b.{0,60}(?:уведоми|без\s+согласия)",
    r"в\s+пределах\s+срока\s+договора\s+аренды.{0,60}(?:уведоми|без\s+согласия)",
]
CESSION_ALLOWED_WITH_CONSENT = [
    # Только формулировки, прямо привязанные к передаче прав / цессии —
    ...
    # добавлено 2026-07-10
    r"(?:уступк\w+|переуступк\w+|перенайм\w*|передач\w+\s+прав)\b.{0,80}(?:по\s+согласовани\w+\s+с\s+арендодател|с\s+разрешени\w+\s+арендодател)",
    r"права\s+и\s+обязанности\s+по\s+договор\w+.{0,80}с\s+(?:письменн\w+\s+)?согласи\w+\s+арендодател",
    r"залог\s+прав\w*\s+аренд\w*.{0,40}с\s+согласи",
]
...
SUBLEASE_FORBIDDEN = [
    ...
    # добавлено 2026-07-10
    r"субаренд\w*.{0,40}(?:не\s+допуска\w+|не\s+разреша\w+|запрещ\w+|исключ\w+)",
    r"(?:не\s+допуска\w+|запрещ\w+)\w*.{0,40}субаренд",
    r"арендатор\s+не\s+(?:вправе|имеет\s+права)\b.{0,60}субаренд",
    r"(?:сдач\w+|сдава\w+|передач\w+|передава\w+)\b.{0,40}в\s+субаренд\w+.{0,40}(?:не\s+допуск|запрещ|не\s+разреш)",
    r"(?:поднайм|поднаем)\w*.{0,40}(?:не\s+допуск|запрещ|не\s+разреш)",
    r"без\s+права\s+(?:сдачи\s+в\s+|передачи\s+в\s+)?субаренд",
]
SUBLEASE_ALLOWED = [
    ...
    # добавлено 2026-07-10
    r"арендатор\s+(?:вправе|имеет\s+право)\b.{0,60}(?:сдава\w+|передава\w+|сдать|передать)\b.{0,20}в\s+субаренд",
    r"(?:сдача|передача)\s+.{0,20}в\s+субаренд\w+\s+(?:допуска\w+|разреш\w+|возможн\w+)",
    r"допуска\w+\s+(?:сдач\w+|передач\w+)\s+.{0,20}в\s+субаренд",
    r"(?:поднайм|поднаем)\w*\s+(?:допуска\w+|разреш\w+)",
]
SUBLEASE_WITH_NOTICE = [
    r"(?:сдава\w+|передава\w+|сдач\w+|передач\w+)\b.{0,30}в\s+субаренд\w+.{0,40}уведоми\w+\s+(?:арендодател|собственник)",
    r"субаренд\w*.{0,40}(?:в\s+уведомительн\w+\s+порядк|уведомив\s+(?:арендодател|собственник))",
    r"без\s+согласия\s+арендодател.{0,60}уведоми\w+.{0,20}субаренд",
]
SUBLEASE_WITH_CONSENT = [
    ...
    # добавлено 2026-07-10
    r"(?:сдава\w+|передава\w+|сдач\w+|передач\w+)\b.{0,30}в\s+субаренд\w+.{0,40}(?:с\s+(?:письменн\w+\s+)?(?:предварительн\w+\s+)?согласи|по\s+согласовани|с\s+разрешени)",
    r"субаренд\w*.{0,60}(?:с\s+предварительн\w+\s+письменн\w+\s+согласи|по\s+письменн\w+\s+согласовани)",
    r"(?:поднайм|поднаем)\w*.{0,40}с\s+согласи",
]
...
def parse_contract(text: str) -> dict:
    ...
    # Нормализуем PDF-текст: схлопываем любые пробельные (вкл.  , переносы,
    # табы) в один пробел и ё→е — иначе \s+ в паттернах рвётся на переносах строк
    # и часть формулировок не находится.
    text = re.sub(r"[\s ]+", " ", text).replace("ё", "е").replace("Ё", "Е")
...
SUBLEASE_LABELS = {
    ...
    "with_notice": "Субаренда — в уведомительном порядке (без согласия)",
