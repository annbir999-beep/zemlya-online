...
def _day30_html(name: str) -> str:
    body = """
    <p>Прошёл месяц с вашей регистрации. Без давления и продаж — хотим напомнить, что
    мы за это время выросли:</p>
    <ul style="line-height:1.7;font-size:14px">
      <li>📈 База лотов: <b>3 600+ активных</b> по всей РФ</li>
      <li>🤖 ИИ-разборы стали точнее, появилась витрина
        <a href="{site}/ai-picks" style="color:#0d9488">«Готовые ИИ-разборы»</a></li>
      <li>📚 Новые статьи в <a href="{site}/blog" style="color:#0d9488">блоге</a></li>
      <li>🎁 <b>Дарим ещё один бесплатный AI-аудит</b> — просто за то, что не забыли о нас</li>
    </ul>
    <p style="font-size:14px">Бесплатный аудит уже на счету. Откройте любой лот с torgi.gov и нажмите кнопку.</p>
    """.replace("{site}", SITE)
    return _wrap(
        name,
        "🎁 Подарок месяца — ещё один бесплатный аудит",
        body,
        "Получить бесплатный аудит →",
        f"{SITE}/audit-lot",
    )
...
DRIP_STEPS = {
    3: ("🔍 Не упустите бесплатный AI-аудит — Торги Земли", _day3_html),
    7: ("💎 Один аудит окупает себя в 100 раз — Торги Земли", _day7_html),
    14: ("⏰ Последний шанс: −50% на AI-аудит — Торги Земли", _day14_html),
    30: ("🎁 Подарок месяца — ещё один бесплатный аудит — Торги Земли", _day30_html),
}
