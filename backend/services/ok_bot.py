...
async def _ensure_subscription(client: httpx.AsyncClient) -> None:
    """Оформляет long-polling подписку на MESSAGE_CREATED (идемпотентно, раз в ~30 мин)."""
    if await _redis().get(SUBSCRIBED_KEY):
        return
    try:
        r = await client.post(
        ...
        if r.status_code == 200:
            await _redis().set(SUBSCRIBED_KEY, "1", ex=1800)
        else:
...
async def poll_updates() -> int:
    ...
    try:
        async with httpx.AsyncClient(timeout=25) as client:
            await _ensure_subscription(client)
