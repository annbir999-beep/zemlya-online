...
2. Аренда → постройка дома → льготный выкуп (ст. 39.20 ЗК)
...
ВАЖНО про проценты выкупа: если ниже приведён блок «РЕГИОНАЛЬНЫЕ УСЛОВИЯ ВЫКУПА» —
бери проценты ТОЛЬКО оттуда и считай деньги по ним. Не подставляй «3% от кадастровой
стоимости» и другие цифры по памяти: в разных регионах ставка отличается в десятки раз
(например, в Московской области выкуп под домом — 60% от КС, а не 3%). Если блока нет —
не называй конкретный процент вообще, напиши «ставка выкупа зависит от региона».
...
def _format_lot_data(lot_dict: dict) -> str:
    ...
    # Региональные проценты выкупа — ПОДСТАВЛЯЕМ ФАКТОМ, а не «зависит от региона».
    # Без этого модель берёт общеизвестные «3% от КС» и промахивается в разы:
    # для Московской области выкуп под домом 60%, а не 3% (проверено 27.07.2026).
    region_code = lot_dict.get("region_code")
    if region_code:
        try:
            from services.regional_data import get_regional_data
            rd = get_regional_data(region_code) or {}
            lb = rd.get("land_buyout") or {}
            if lb:
                lines.append(
                    "\nРЕГИОНАЛЬНЫЕ УСЛОВИЯ ВЫКУПА (официальные, использовать ТОЛЬКО их):"
                    f"\n- Выкуп без торгов по ст. 39.18: {lb.get('direct_3918', '—')}% от КС"
                    f"\n- Выкуп под жилым домом по ст. 39.20: {lb.get('house_3920', '—')} от КС"
                    f"\n- Выкуп под иным объектом по ст. 39.20: {lb.get('other_3920', '—')} от КС"
                )
            if rd.get("agro_buyout") is not None:
                lines.append(f"- Выкуп сельхозземли после 3 лет аренды: {rd['agro_buyout']}% от КС")
            if rd.get("kfh_house_allowed") is not None:
                lines.append(
                    f"- Дом на землях КФХ: {'разрешён' if rd['kfh_house_allowed'] else 'запрещён'}"
                )
        except Exception as e:
            print(f"[ai] regional_data lookup failed for {region_code}: {type(e).__name__}: {e}")
...
def lot_to_ai_dict(lot) -> dict:
    ...
    return {
        ...
        "region_code": lot.region_code,  # нужен для подстановки региональных ставок выкупа
        "address": lot.address,
