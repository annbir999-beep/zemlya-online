...
#
# После «начальн...» слово не фиксируем: у аренды пишут «от начального размера
# годовой арендной платы», у продажи — «от начальной цены». Требование именно
# «цены» отсекало все арендные лоты, а это большая часть базы.
_PERCENT = re.compile(
    r"задат\w*[^.\n]{0,40}?(\d{1,3}(?:[.,]\d{1,2})?)\s*%\s*от\s+начальн\w*",
    re.I,
)
