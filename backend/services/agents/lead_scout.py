...
FORUM_FEEDS = [
    # ForumHouse, раздел «Земельно-имущественные отношения» — там ВРИ, кадастр,
    # аренда, выкуп, торги и сделки. Именно наши вопросы: в первой же выдаче
    # были «Стоимость аренды участка у государства после аукциона» и
    # «Перераспределение земельного участка».
    ("ForumHouse · земля", "https://www.forumhouse.ru/forums/103/index.rss"),
    # Общая лента форума — на случай, если земельная тема заведена в другом
    # разделе. Шума много, поэтому идёт второй и фильтруется так же.
    ("ForumHouse · всё", "https://www.forumhouse.ru/forums/-/index.rss"),
]
