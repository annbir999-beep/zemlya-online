...
# Лимиты задаются задаче персонально: глобальные 1200/1500 из worker.py
# рассчитаны на быстрые periodic-задачи, а этот прогон честно долгий (запрос к
# PKK + пауза + повторы + вторая проверка по координатам). Убитый по мягкому
# лимиту прогон опаснее долгого: SoftTimeLimitExceeded прилетает прямо в
# loop.run_until_complete, мимо корутины, поэтому finally в
# _with_engine_cleanup не доводит engine.dispose() до конца и луп закрывается
# с живыми соединениями asyncpg и таймер-таской curl_cffi. Осиротевший пул
# потом ломает СЛЕДУЮЩУЮ задачу того же форк-воркера
# (18.08: InterfaceError «the underlying connection is closed» через 9 часов
# после убийства). Жёсткий лимит держим ниже ttl лока _single_flight (3300),
# иначе следующий запуск по расписанию упрётся в лок от мёртвого прогона.
@celery_app.task(
    bind=True,
    max_retries=3,
...
def enrich_with_rosreestr(self, batch_size: int = 120):
    """Обогащаем участки данными из Росреестра (кадастровые данные).

    batch_size считаем от замера, а не от теории: 12.08 прогон на 300 лотах шёл
    19 минут — около 4 секунд на участок, хотя сам запрос к PKK отвечает за 0.3.
    Разницу съедают пауза, повторы и вторая проверка по координатам. Значит в
    лимит 20 минут надёжно помещается ~200 лотов.

    Было 2000 «по расчёту 12 минут» — на практике прогон каждый раз упирался в
    жёсткий лимит 25 минут и умирал. За сутки это 4800 участков, backlog в 6500
    закрывается за полтора дня.

    Порцию задаёт beat (worker.py), и до 17.08 там оставалось 1000 — правку
    12.08 внесли в дефолт функции, а расписание не тронули. Итог: прогон
    выбирал лимит, ретраился ещё трижды по той же порции и занимал воркер
    ~80 минут при часовом расписании, поэтому каждый следующий запуск сутки
    подряд писал «предыдущий прогон ещё идёт». Обогащение не двинулось ни разу.

    18.08 замер снова разошёлся с оценкой: не 4 секунды на участок, а 7.7 —
    прогон 00:30 успел 155 из 200 и был убит по мягкому лимиту. Остаток
    очереди (3955 лотов, ни одного ни разу не пробованного) — это повторные
    ротации «непробиваемых» участков, на которые PKK отвечает 404 после всех
    повторов, поэтому они и идут дольше успешных. Отсюда 120 на прогон при
    персональном лимите 40 минут: обычный прогон занимает треть лимита, и
    медленный день не убивает задачу на середине await.
    """
    with _single_flight("enrich_with_rosreestr") as acquired:
        ...
        try:
            ...
            # Прогресс коммитится каждые 50 участков — набранное уже в базе.
            # Повторять ту же порцию бессмысленно: она не влезла и не влезет,
            # а ретраи держат лок и голодают следующий запуск по расписанию.
            print("[rosreestr] прогон не уложился в лимит — остаток возьмёт следующий запуск")
            raise
        except Exception as exc:
            raise self.retry(exc=exc)
...
async def _enrich_rosreestr(batch_size: int = 2000):
    ...
    async with SessionLocal() as db:
        ...
        for lot in lots:
            ...
            # Сохраняем по ходу, а не одним разом в конце. Прогон упирается в
            # жёсткий лимит времени и убивается сигналом — при единственном
            # commit в конце вся набранная работа пропадала. Именно поэтому
            # координаты не прибавлялись сутками, хотя задача исправно
            # запускалась и PKK отвечал за доли секунды.
...
@celery_app.task(bind=True, max_retries=1, default_retry_delay=600)
def enrich_nearby_features(self, batch_size: int = 35):
    """Обогащает лоты данными из OSM Overpass API: водоёмы / лес / трассы / н.п. / ж/д.

    batch_size от замера живого прогона, а не от паузы в коде: между лотами
    стоит sleep 2.5с, но по логам 17.08 двадцать лотов проходят за 8-10 минут —
    то есть 25-30 секунд на лот. Разницу съедает сам Overpass: основной endpoint
    отдаёт 429/504, httpx ждёт свои 40 секунд и уходит на запасной. При порции
    100 прогон физически не может уложиться в лимит 20 минут — каждый час он
    выбирал лимит, ретраился и выбирал снова. 35 берём по худшему замеру
    (30.7с/лот → 18 минут), не по среднему: на среднем 25с порция 40 всё ещё
    иногда срывалась бы в лимит. Очередь и так почти пуста — типичный прогон
    берёт 24-67 лотов.
    """
    try:
        _run(_enrich_nearby_features(batch_size))
    except SoftTimeLimitExceeded:
        # nearby_features коммитится по одному лоту — набранное уже в базе.
        print("[nearby] прогон не уложился в лимит — остаток возьмёт следующий запуск")
        raise
    except Exception as exc:
        raise self.retry(exc=exc)
